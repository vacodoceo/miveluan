...
export async function listExamsPDFs(
  accessToken: string
): Promise<ExamFile[]> {
  try {
    const folderId = await getDriveforlderIdIfExists(EXAM_FOLDER_NAME, accessToken);
    if (!folderId) {
      return [];
    }
    ...
    const response = await fetch(
      `https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(query)}&fields=${fields}&orderBy=${orderBy}`,
      {
        headers: {
...
export async function uploadExam(
  ...
  try {
    ...
    const folderId = await createDriveFolderIfNotExists(EXAM_FOLDER_NAME, accessToken);
